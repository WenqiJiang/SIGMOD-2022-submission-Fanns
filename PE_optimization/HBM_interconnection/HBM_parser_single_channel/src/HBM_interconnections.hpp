...
#pragma HLS interface port=return register
    ...
    // the input data layout is big endian: 
    //   vec_ID0, PQ_code0 (511, 352)
    //   vec_ID1, PQ_code1 (351, 192)
    //   vec_ID2, PQ_code2 (191, 32)
    //   padding (4 bytes) (31, 0)

    ap_uint<32> tmp_int0 = in.range(511, 480);
    out.PQ_0.vec_ID = *((int*)(&tmp_int0));
    for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
        ap_uint<8> tmp_char = in.range(480 - 1 - i * 8, 480 - 8 - i * 8);
        out.PQ_0.PQ_code[i] = *((unsigned char*)(&tmp_char));
    }

    ap_uint<32> tmp_int1 = in.range(511 - 160, 480 - 160);
    out.PQ_1.vec_ID = *((int*)(&tmp_int1));
    for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
        ap_uint<8> tmp_char = in.range(480 - 160 - 1 - i * 8, 480 - 160 - 8 - i * 8);
        out.PQ_1.PQ_code[i] = *((unsigned char*)(&tmp_char));
    }

    ap_uint<32> tmp_int2 = in.range(511 - 320, 480 - 320);
    out.PQ_2.vec_ID = *((int*)(&tmp_int2));
    for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
        ap_uint<8> tmp_char = in.range(480 - 320 - 1 - i * 8, 480 - 320 - 8 - i * 8);
