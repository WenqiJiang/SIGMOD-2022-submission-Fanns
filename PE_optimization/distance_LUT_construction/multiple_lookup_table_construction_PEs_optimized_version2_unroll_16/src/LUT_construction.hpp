...
void lookup_table_construction_PE(
    ...
    // it takes 64 BRAMs in total to store 256 * 128 * 32 bit, thus naturally unrolled
    float product_quantizer[K][M][D / M];
#pragma HLS resource variable=product_quantizer core=RAM_1P_BRAM

    // DRAM PQ quantizer format: 16 (M) x 256 (K) x 8 (D/M)
    //   the read order is different to storage order
    for (int m = 0; m < M; m++) {
        for (int k = 0; k < K; k++) {
            for (int j = 0; j < D / M; j++) {
#pragma HLS pipeline II=1
                product_quantizer[k][m][j] = s_product_quantizer_init.read();
            }
        }
    }
...
#pragma HLS array_partition variable=result_distance complete
...
#pragma HLS UNROLL factor=2
                    for (int j = 0; j < D / M; j++) {
#pragma HLS UNROLL
                        L1_dist[m][j] = residual_center_vector[m][j] - product_quantizer[k][m][j];
                    }
