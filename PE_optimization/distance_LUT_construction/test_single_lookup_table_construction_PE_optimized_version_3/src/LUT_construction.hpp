...
void parallel_L2_distance(
    ...
    float in_array_B[data_width], 
    float out_array[data_width]);
    
template<typename T, const int data_width>
T reduction_sum_LUT_construction(T array[data_width]);

template<>
void parallel_L2_distance<8>(
    float in_array_A[8], 
    float in_array_B[8], 
    float out_array[8]) {
#pragma HLS inline
    ...
    for (int i = 0; i < 8; i++) {
#pragma HLS UNROLL
        float diff = in_array_A[i] - in_array_B[i];
        out_array[i] = diff * diff;
    }
}

template<>
float reduction_sum_LUT_construction<float, 8>(float array[8]) {
#pragma HLS inline
...
void partition_center_vectors_table_construction_PE(
    hls::stream<struct_8_float_t> &s_center_vectors_table_construction_PE,
...
#pragma HLS pipeline II=1
        ...
        for (int nprobe_id = 0; nprobe_id < nprobe_per_PE; nprobe_id++) {
            ...
            float partial_center_vector[D / M]; 
#pragma HLS array_partition variable=partial_center_vector complete
...
#pragma HLS array_partition variable=partial_residual_center_vector complete
            ...
            parallel_L2_distance<8>(
                partial_center_vector, 
                partial_query_vector_local, 
                partial_residual_center_vector);
