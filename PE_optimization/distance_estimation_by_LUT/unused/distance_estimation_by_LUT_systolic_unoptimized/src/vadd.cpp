...
#pragma HLS pipeline II=1
                ...
                distance_LUT_PQ16_t reg_in = s_distance_LUT_in.read();
                distance_LUT_PQ16_t reg_out = reg_in;
                distance_LUT_PQ16_t dist_row = reg_in;
                s_distance_LUT_out.write(reg_out);
                ...
                // col 8 ~ 15
                distance_LUT[0][row_id + 256] = dist_row.dist_8; 
                distance_LUT[1][row_id + 256] = dist_row.dist_9; 
                distance_LUT[2][row_id + 256] = dist_row.dist_10; 
                distance_LUT[3][row_id + 256] = dist_row.dist_11; 
                distance_LUT[4][row_id + 256] = dist_row.dist_12; 
                distance_LUT[5][row_id + 256] = dist_row.dist_13; 
                distance_LUT[6][row_id + 256] = dist_row.dist_14; 
                distance_LUT[7][row_id + 256] = dist_row.dist_15;
            }

//             // Stage A: init distance LUT
//             for (unsigned char row_id = 0; row_id < K; row_id++) {
// #pragma HLS pipeline II=1
...
//                 // col 8 ~ 15
//                 distance_LUT[0][((short) row_id) + 256] = dist_row.dist_8; 
//                 distance_LUT[1][((short) row_id) + 256] = dist_row.dist_9; 
//                 distance_LUT[2][((short) row_id) + 256] = dist_row.dist_10; 
//                 distance_LUT[3][((short) row_id) + 256] = dist_row.dist_11; 
//                 distance_LUT[4][((short) row_id) + 256] = dist_row.dist_12; 
//                 distance_LUT[5][((short) row_id) + 256] = dist_row.dist_13; 
//                 distance_LUT[6][((short) row_id) + 256] = dist_row.dist_14; 
//                 distance_LUT[7][((short) row_id) + 256] = dist_row.dist_15;
//             }
//             // Stage A: init distance LUT
//             for (short row_id = 0; row_id < K; row_id++) {
// #pragma HLS pipeline II=1
...
#pragma HLS resource variable=out.dist core=FAddSub_fulldsp
                out.dist = 
                    ...
                    distance_LUT[0][((short)PQ_local.PQ_code[8]) + 256] + 
                    distance_LUT[1][((short)PQ_local.PQ_code[9]) + 256] + 
                    distance_LUT[2][((short)PQ_local.PQ_code[10]) + 256] + 
                    distance_LUT[3][((short)PQ_local.PQ_code[11]) + 256] + 
                    distance_LUT[4][((short)PQ_local.PQ_code[12]) + 256] + 
                    distance_LUT[5][((short)PQ_local.PQ_code[13]) + 256] + 
                    distance_LUT[6][((short)PQ_local.PQ_code[14]) + 256] + 
                    distance_LUT[7][((short)PQ_local.PQ_code[15]) + 256];
