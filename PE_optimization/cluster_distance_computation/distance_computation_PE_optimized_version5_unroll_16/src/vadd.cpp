...
void vadd(  
    ...
    float out_PLRAM[QUERY_NUM * CENTROIDS_PER_DISTANCE_COMPUTATION_PE]
    )
{
...
#pragma HLS stream variable=s_partial_cell_distance depth=4
    ...
    write_result<QUERY_NUM * CENTROIDS_PER_DISTANCE_COMPUTATION_PE>(s_partial_cell_distance, out_PLRAM);
}
...
void broadcast_init_centroid_vectors(
    ...
    for (int i = 0; i < CENTROIDS_PER_DISTANCE_COMPUTATION_PE * D; i++) {
