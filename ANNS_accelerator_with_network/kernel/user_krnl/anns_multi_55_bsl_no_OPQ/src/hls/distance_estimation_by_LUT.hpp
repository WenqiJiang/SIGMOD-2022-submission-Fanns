...
template<const int stream_num, const int query_num, const int nprobe>
void replicate_s_scanned_entries_every_cell_Dummy(
    hls::stream<int>& s_scanned_entries_every_cell_Dummy,
    hls::stream<int> (&s_scanned_entries_every_cell_Dummy_replicated)[stream_num]) {
...
// #pragma HLS RESOURCE variable=s_scanned_entries_every_cell_PQ_lookup_computation_replicated core=FIFO_SRL
    ...
    hls::stream<int> s_scanned_entries_every_cell_Dummy_replicated[SORT_GROUP_NUM * 16 - 3 * HBM_CHANNEL_NUM]; // 32 - 3 * 10 = 2
...
// #pragma HLS RESOURCE variable=s_scanned_entries_every_cell_Dummy_replicated core=FIFO_SRL
    ...
    replicate_s_scanned_entries_every_cell_Dummy<SORT_GROUP_NUM * 16 - 3 * HBM_CHANNEL_NUM, query_num, nprobe>(
...
// #pragma HLS RESOURCE variable=s_last_element_valid_PQ_lookup_computation core=FIFO_SRL
    ...
    // WENQI: The following logic is not comprehensively tested, please double-check
    //   before integrating it into the automatic code generation tool
#if SORT_GROUP_NUM == 1
    // head PE 1~15 
    for (int j = 1; j < 3 * HBM_CHANNEL_NUM; j++) {
#pragma HLS UNROLL
        PQ_lookup_computation<query_num, nprobe>(
            // input streams
    ...
    dummy_distance_LUT_consumer<query_num, nprobe>(
        s_distance_LUT_systolic[3 * HBM_CHANNEL_NUM - 1]);

    for (int j = 3 * HBM_CHANNEL_NUM; j < 16; j++) {
#pragma HLS UNROLL
        dummy_PQ_result_sender<query_num, nprobe>(
            s_scanned_entries_every_cell_Dummy_replicated[j - 3 * HBM_CHANNEL_NUM], 
            s_single_PQ_result[0][j]);
    }
...
#pragma HLS UNROLL
    ...
    // tail PE 16~26 
    for (int j = 0; j < 3 * HBM_CHANNEL_NUM - 16 * (SORT_GROUP_NUM - 1); j++) {
#pragma HLS UNROLL
        PQ_lookup_computation<query_num, nprobe>(
            // input streams
    ...
    for (int j = 3 * HBM_CHANNEL_NUM - 16 * (SORT_GROUP_NUM - 1); j < 16; j++) {
#pragma HLS UNROLL
        dummy_PQ_result_sender<query_num, nprobe>(
            s_scanned_entries_every_cell_Dummy_replicated[j - (3 * HBM_CHANNEL_NUM - 16 * (SORT_GROUP_NUM - 1))], 
            s_single_PQ_result[SORT_GROUP_NUM - 1][j]);
    }
