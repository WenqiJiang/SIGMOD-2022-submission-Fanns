     ...
     do{
          ...
          else
          {
               if (!s_axis_tcp_tx_status.empty())
               {
                    ...
                    resp.remaining_space = txStatus_pkt.data(61,32);
                    resp.error = txStatus_pkt.data(63,62);
                    currentPkgWordCnt = (resp.length + 512/8 -1) >> 6;
...
// Wenqi: forward the session ID to the sender (for the case of a single connection)
void sendDataSingleCon(hls::stream<pkt32>& m_axis_tcp_tx_meta, 
               hls::stream<pkt512>& m_axis_tcp_tx_data, 
               hls::stream<pkt64>& s_axis_tcp_tx_status,
               hls::stream<ap_uint<512> >& s_data_in,
               hls::stream<ap_uint<16> >& stream_sessionID,
               ap_uint<64> expectedTxByteCnt, 
               int pkgWordCount
                )
{
     ...
     do{
          ...
          if (first_round & !s_data_in.empty() & !stream_sessionID.empty())
          {
               sessionID = stream_sessionID.read();
               tx_meta_pkt.data(15,0) = sessionID;
               tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
               m_axis_tcp_tx_meta.write(tx_meta_pkt);
               ...
               hls::stream<ap_uint<16> >& stream_sessionID)
{
     ...
     do{
          if (!s_axis_tcp_notification.empty())
          {
               ...
               if (first_round) {
                    stream_sessionID.write(sessionID);
                    first_round = false;
               }
...
// Wenqi: forward the session ID to the sender (for the case of MULTI connections)
void recvData_handshake_freerun(
     hls::stream<pkt128>& s_axis_tcp_notification, 
     hls::stream<pkt32>& m_axis_tcp_read_pkg,
     hls::stream<ap_uint<16> >& s_nextRxPacketLength_out,
     hls::stream<ap_uint<16> >& s_sessionID_out)
...
void recvData_consumeData_freerun(
               hls::stream<pkt16>& s_axis_tcp_rx_meta, 
               hls::stream<pkt512>& s_axis_tcp_rx_data,
               hls::stream<ap_uint<512> >& s_data_out )
{
...
// WENQI: count next packet length in read loop
void recvData_consumeData_ver2(ap_uint<64> expRxBytePerSession, 
               hls::stream<pkt16>& s_axis_tcp_rx_meta, 
               hls::stream<pkt512>& s_axis_tcp_rx_data,
               hls::stream<ap_uint<16> >& nextRxPacketLength,
               hls::stream<ap_uint<16> >& s_nextRxPacketLength_out,
               hls::stream<ap_uint<512> >& s_data_out )
{
     ...
     do{
          if (!s_axis_tcp_rx_meta.empty() & !nextRxPacketLength.empty())
          {
               ...
               s_nextRxPacketLength_out.write(pkg_counter);
               rxByteCnt = rxByteCnt + length;
          }
...
#pragma HLS dataflow disable_start_propagation
     ...
     recvData_handshake_freerun(
               s_axis_tcp_notification, 
               m_axis_tcp_read_pkg,
               s_nextRxPacketLength_out,
               s_sessionID_out);

     recvData_consumeData_freerun(
               s_axis_tcp_rx_meta, 
               s_axis_tcp_rx_data,
               s_data_out);
}
...
#pragma HLS dataflow disable_start_propagation
     ...
     recvData_handshake_ver2(expRxBytePerSession, 
               s_axis_tcp_notification, 
               m_axis_tcp_read_pkg,
               s_nextRxPacketLength_consumeData,
               s_sessionID_out);

     recvData_consumeData_ver2(expRxBytePerSession, 
               s_axis_tcp_rx_meta, 
               s_axis_tcp_rx_data,
               s_nextRxPacketLength_consumeData,
               s_nextRxPacketLength_out,
